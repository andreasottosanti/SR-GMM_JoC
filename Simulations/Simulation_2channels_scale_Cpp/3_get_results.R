...
for(k in 1:length(directories)){
  ...
  transform_labels <- function(label, signal = 2, noise_to_signal = F){
    ...
    new_label <- paste("Scales: (", paste(numbers_vector[1], numbers_vector[2], sep = ", "),")",sep="")
    new_label
  }
...
ggsave(filename = paste(dirname(rstudioapi::getSourceEditorContext()$path), "/Runtime_scale",".pdf",sep=""),
       plot = runtime_plot, width = 12, height = 9)
ggsave(filename = paste(dirname(rstudioapi::getSourceEditorContext()$path), "/Runtime_per_iter_scale",".pdf",sep=""),
       plot = runtime_plot_std, width = 12, height = 9)
ggsave(filename = paste(dirname(rstudioapi::getSourceEditorContext()$path), "/N_iter_scale",".pdf",sep=""),
