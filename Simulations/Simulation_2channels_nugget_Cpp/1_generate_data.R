...
nugget_values <- c(0, 1, 3)  # the values of nugget effects considered
nugget_values_grid <- expand.grid(nugget_values, nugget_values)
...
for(i in 1:nrow(nugget_values_grid)){
  combination_type <- paste(nugget_values_grid[i,1], nugget_values_grid[i,2], sep="_")
  ...
  for(j in 1:N_data_per_simulation){
    ...
    # Z = 1
    label1 <- paste("expo, tau=",tau,", phi=",phi,", xi=",nugget_values_grid[i,1],sep = "")
    K1 <- tau * exp(-as.matrix(dist(C))/phi) + diag(nugget_values_grid[i,1], p)
    row.names(K1) <- colnames(K1) <- NULL
    y1 <- mvtnorm::rmvnorm(n = n[1], sigma = K1)

    # Z = 2
    label2 <- paste("gaus, tau=",tau,", phi=",phi,", xi=",nugget_values_grid[i,2],sep = "")
    K2 <- tau * exp(-as.matrix(dist(C))^2/phi^2) + diag(nugget_values_grid[i,2], p)
