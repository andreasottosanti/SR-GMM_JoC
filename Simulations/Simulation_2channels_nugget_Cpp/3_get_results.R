...
for(k in 1:length(directories)){
  ...
  transform_labels <- function(label, signal = 2, noise_to_signal = F){
    ...
    if(!noise_to_signal){
      ratio <- round(signal/numbers_vector, 3)
      new_label <- paste("StN ratios: (", paste(ratio[1], ratio[2], sep = ", "),")",sep="")
    } else {
      ratio <- round(numbers_vector/signal, 3)
      new_label <- paste("NtS ratios: (", paste(ratio[1], ratio[2], sep = ", "),")",sep="")
    }
    new_label
  }
...
Comp_time$simulation_name <- factor(Comp_time$simulation_name, levels = levels(values$setup_relabelled))
